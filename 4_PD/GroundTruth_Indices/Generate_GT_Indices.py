...
if 0: #use this branch if occupancies assigned from previously-generated occupancy map
    ...
    states_gt = 20 #ground-truth number of states along each degree of freedom (assumed symmetric)
    tau = 1 #see '0_Data_Inputs/Pristin_addTauSNR' for more information
    ss_gt = 400 #total number of images in dataset
    ...
    np.save(os.path.join(pyDir, 'CM1_Indices_tau%s.npy' % tau), CM1_idx)
    if 0:
        CM1_final = np.load('CM1_Indices_tau%s.npy' % tau, allow_pickle=True)
        print(CM1_final)
    ...
    np.save(os.path.join(pyDir, 'CM2_Indices_tau%s.npy' % tau), CM2_idx)            
    if 0:
        CM1_final = np.load('CM2_Indices_tau%s.npy' % tau, allow_pickle=True)
        print(CM1_final)
