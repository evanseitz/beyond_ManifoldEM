...
if 1: #plot distances of state_01_01 to all others
    ...
    ax.plot(x_values2, y_values2, c='red', linewidth=1.5, zorder=-1, label='$\mathrm{CM_1}$')
    ax.plot(x_values1, y_values1, c='blue', linewidth=1.5, zorder=-1, label='$\mathrm{CM_2}$')
    ...
    plt.tight_layout()
    #plt.show()
    fig.savefig(os.path.join(pyDir,'_figure_assets_/Fig8_PDB_dists.pdf'), dpi=600)
    plt.clf()
...
if 1: #ordered array 2D manifold subspaces
    ...
    idx_col1 = 0  
    idx_col2 = 6
    for v1 in range(1,7):#(2*int(dimCols/2)+1)):
    ...
    fig.savefig(os.path.join(pyDir,'_figure_assets_/Fig6_%s.pdf' % figname), dpi=600)
    plt.clf()
